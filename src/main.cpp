...
#define S_SENSOR8       108
#define S_JSON          110
#define S_DEVICES       120
#define S_DEVICE        121
...
void ShowSensor1() {
  ...
  switch(P[ActivePeer].S[ActiveSensor].Type) {
    case SENS_TYPE_AMP: 
...
bool isAPPDC(){
  return (isPDC(P[ActivePeer].Type));
}
bool isAPBattery(){
  return (P[ActivePeer].Type == BATTERY_SENSOR);
}
bool isASSensorVolt() {
  return (P[ActivePeer].S[ActiveSensor].Type == SENS_TYPE_VOLT);
}
bool isASSensorAmp() {
  return (P[ActivePeer].S[ActiveSensor].Type == SENS_TYPE_AMP);
}
bool isASSensorSwitch() {
  return (P[ActivePeer].S[ActiveSensor].Type == SENS_TYPE_SWITCH);
}
int  ASensorType() {
  return (P[ActivePeer].S[ActiveSensor].Type);
}
bool SensorChanged(int Start, int Stop=0) {
  int ret = false;
